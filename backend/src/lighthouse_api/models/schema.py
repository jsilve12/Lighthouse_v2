...
class SchemaVersion(UUIDMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("folder_id", "major_version", "minor_version", name="uq_schema_versions_folder_version"),
    )
    ...
    data_location_pattern: Mapped[str | None] = mapped_column(
        ...
        comment="Regex pattern matching GCS paths this schema applies to"
    )

    folder: Mapped["Folder"] = relationship("Folder", back_populates="schema_versions")
    fields: Mapped[list["SchemaField"]] = relationship(
        "SchemaField", back_populates="schema_version", cascade="all, delete-orphan"
    )
...
class SchemaField(UUIDMixin, Base):
    ...
    parent_field_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("schema_fields.id"), nullable=True
    )
    ...
    parent: Mapped["SchemaField | None"] = relationship(
        "SchemaField", back_populates="children", remote_side="SchemaField.id"
    )
